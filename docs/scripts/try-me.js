...
─────────────────────────────────────────────────────── */
const DEFAULT_TEXT = '←←← חבית‭ ‬בירה? Punk IPA 13% מארז‭ ‬סודה 12 ליטר + 6 כוסות & ארגז יין‭ ‬כתום 12.3% Midbar הפוך שיבולת {₪28} • מים (₪9) וויסקי Glenfiddich 21 בחנויות אספרסו כפול מפולי Arabica →→ חֲלִיטַת תֵּה Wissotzky החל מ־2€ ¾ של צ׳ייסר Noah №12 ב־18:25';

const TEXT_PRESETS = {
  paragraph: { text: DEFAULT_TEXT, size: null, weight: 900, lineHeight: null },
  line:      { text: '<span style="font-feature-settings: \'ss01\' 1;">חליטת תה</span> Wissotzky בטעם\nHoney קָמוֹמִיל', size: 245, weight: 900, lineHeight: null },
  word:      { text: 'אספרסו Arabica', size: 450, weight: 900, lineHeight: 0.79 },
};
...
─────────────────────────────────────────────────────── */
const PALETTES = [
  { label: 'ראשי',    textColor: null,      bgColor: '#0F0E0C', useGreen: true },
  { label: 'שחור/לבן', textColor: '#0F0E0C', bgColor: '#f5f5f5' },
  { label: 'ורוד/צהוב', textColor: '#ffe600', bgColor: '#ed3d66' },
  { label: 'סגול/ירוק', textColor: '#87c540', bgColor: '#4A00E0' },
  { label: 'כחול/כתום', textColor: '#ff7340', bgColor: '#0055FF' },
  { label: 'תכלת/אדום', textColor: '#ed3d66', bgColor: '#00E5FF' },
  { label: 'אקראי',   textColor: null,      bgColor: null },
];
...
function initTextPresets() {
  const btns = [...document.querySelectorAll('[data-text-preset]')];
  if (!btns.length) return;
  btns.forEach(btn => {
    btn.addEventListener('click', () => {
      const key = btn.dataset.textPreset;
      ...
      canvas.innerHTML = preset.text;

      if (preset.size !== null) {
        sizeRange.value = preset.size;
        sizeRange.dispatchEvent(new Event('input'));
      }

      if (preset.lineHeight !== null && preset.lineHeight !== undefined) {
        lhRange.value = preset.lineHeight;
        lhRange.dispatchEvent(new Event('input'));
      }

      if (preset.weight !== null && preset.weight !== undefined) {
        weightSlider.value = preset.weight;
        weightSlider.dispatchEvent(new Event('input'));
        setActivePreset(preset.weight);
      }
    });
  });
  ...
  const sliderInputs = document.querySelectorAll('#size-range, #lh-range, #ls-range');
...
function initSliders() {
  ...
  bindSliderPair(lsRange, lsNum, val => {
    ...
    lsNum.value = val < 0 ? str.replace('-0.', '-.') : str;
  });
...
function updateColorUI() {
  ...
  if (bgColorPicker)   bgColorPicker.value   = state.bgColor;
  if (textHexInput)    textHexInput.value    = state.textColor;
  if (bgHexInput)      bgHexInput.value      = state.bgColor;
}

function initColors() {
  // Swap button
  const swapBtn = document.getElementById('tp-color-swap');
  if (swapBtn) {
  ...
  const bank = ['#ed3d66', '#ffe600', '#ff7340', '#87c540', '#0F0E0C', '#f5f5f5', '#4A00E0', '#0055FF', '#00E5FF'];
  const getRandom = () => bank[Math.floor(Math.random() * bank.length)];
  ...
  PALETTES.forEach((pal, i) => {
    ...
    if (pal.label === 'אקראי') {
      // Pre-generate from bank for display; regenerate on each click
      let newBg  = getRandom();
      let newTxt = getRandom();
      while (newTxt === newBg) newTxt = getRandom();
      let current = { bgColor: newBg, textColor: newTxt };
      ...
      btn.addEventListener('click', () => {
        ...
        newTxt = getRandom();
        while (newTxt === newBg) newTxt = getRandom();
        current = { bgColor: newBg, textColor: newTxt };
        btn.style.setProperty('--swatch-bg',   current.bgColor);
        btn.style.setProperty('--swatch-text', current.textColor);
      });
    } else {
